...
def analisi_comparata_completa_con_asimmetria_graduale(
    ...
    if sa['kurtosis'] > 3 and sb['kurtosis'] > 3:
        r.append(f"Entrambi ({a} e {b}) mostrano code pesanti: forti variazioni dei prezzi si verificano più frequentemente.")
    elif sa['kurtosis'] > 3 and sb['kurtosis'] <= 3:
        r.append(f"{a} è maggiormente soggetto a variazioni estreme, mentre {b} segue una distribuzione più simile alla normale.")
    elif sa['kurtosis'] <= 3 and sb['kurtosis'] > 3:
        r.append(f"{b} è soggetto a eventi estremi, mentre {a} segue una distribuzione più simile alla normale.")
    else:
